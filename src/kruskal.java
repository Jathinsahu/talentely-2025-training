...
public class kruskal {
    ...
    void kruskal_mst(city[]c,int[][]mat){
        int count=0;
        ...
        while(count < c.length){
            ...
            vis[ind]=true;
            for (int i = 0; i < c.length; i++) {
                if(vis[i]==false && mat[ind][i] !=0 && res[i].wt > mat[ind][i]){
                    res[i].wt = mat[ind][i];
                    res[i].prev = ind;
                }
            }
    ...
    void printer(city[] c){
        ...
        for (int i = 1; i < res.length; i++) {
            System.out.println(c[res[i].prev].ccode+" -> "+c[i].ccode+" : "+res[i].wt);
        }
    }
    ...
    public static void main(String[] args) {
        ...
        kruskal pr =new kruskal(cities);
        pr.kruskal_mst(cities,g.adj);
        pr.printer(cities);
    }
}
